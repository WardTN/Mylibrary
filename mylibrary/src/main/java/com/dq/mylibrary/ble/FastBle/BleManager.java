package com.dq.mylibrary.ble.FastBle;
...
import com.dq.mylibrary.ble.FastBle.bluetooth.MultipleBluetoothController;
import com.dq.mylibrary.ble.FastBle.bluetooth.SplitWriter;
...
import com.dq.mylibrary.ble.FastBle.scan.BleScanner;
...
public class BleManager {
    ...
    public BluetoothGatt connect(BleDevice bleDevice, BleGattCallback bleGattCallback) {
        // 检查BleGattCallback是否为null，如果为null则抛出异常
        if (bleGattCallback == null) {
            throw new IllegalArgumentException("BleGattCallback can not be Null!");
        }

        // 检查蓝牙是否已启用，如果未启用则记录日志并通知回调失败
        if (!isBlueEnable()) {
            BleLog.e("Bluetooth not enable!");
     ...
     * 检查设备是否支持蓝牙低功耗（BLE）
     *
     * @return 如果设备支持蓝牙低功耗返回true，否则返回false
     */
    public boolean isSupportBle() {
        // 通过PackageManager查询设备是否具有蓝牙低功耗功能
    ...
    /**
     * 方法1：通过蓝牙适配器直接打开蓝牙
     * 此方法检查蓝牙适配器是否已初始化，如果已初始化，则启用蓝牙
     * 注意：此方法不会检查蓝牙是否已经处于启用状态，也不会处理权限问题
     */
     ...
     * 检查蓝牙是否可用
     * <p>
     * 此方法用于判断设备上的蓝牙功能是否已启用它首先检查bluetoothAdapter对象是否不为null，
     * 以确保蓝牙适配器可用然后调用isEnabled方法检查蓝牙功能是否已开启
     *
     * @return boolean 表示蓝牙功能是否可用如果返回true，表示蓝牙功能已启用；
     * 如果返回false，表示蓝牙功能未启用或bluetoothAdapter对象为null
     */
