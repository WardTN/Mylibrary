...
class CoroutineClient(
    ...
    fun connect(
        device: Device, mtu: Int = 0, timeoutMillis: Long = 6000,
        ...
        val connectFlow = channelFlow {
            if (connect(device, mtu, timeoutMillis, 0) { state ->
                ...
                awaitClose {
                    dqLog("$logTag --> connectFlow closed")
                }
            }
        }.flowOn(Dispatchers.IO)
    ...
    private fun ProducerScope<ConnectionState>.checkReconnect(
    ...
    fun receiveData(uuid: UUID? = null): Flow<ByteArray> {
        return channelFlow {
            if (receiveData(uuid, ::trySend)) {
                awaitClose {
                    dqLog("$logTag --> receiveDataFlow closed")
                    cancelReceive(uuid)
                }
            }
        }.flowOn(Dispatchers.IO)
