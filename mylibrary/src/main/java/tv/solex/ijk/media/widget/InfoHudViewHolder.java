...
import tv.solex.ijk.media.player.MediaPlayerProxy;
                ...
                case MSG_UPDATE_HUD: {
                    ...
                    switch (vdec) {
                        case IjkMediaPlayer.FFP_PROPV_DECODER_AVCODEC:
                            setRowValue(R.string.endo_vdec, "avcodec");
                            break;
                        case IjkMediaPlayer.FFP_PROPV_DECODER_MEDIACODEC:
                            setRowValue(R.string.endo_vdec, "MediaCodec");
                            break;
                        default:
                            setRowValue(R.string.endo_vdec, "");
                            break;
                    }

                    float fpsOutput = mp.getVideoOutputFramesPerSecond();
                    float fpsDecode = mp.getVideoDecodeFramesPerSecond();
                    setRowValue(R.string.endo_fps, String.format(Locale.US, "%.2f / %.2f", fpsDecode, fpsOutput));
                    ...
                    setRowValue(R.string.endo_v_cache, String.format(Locale.US, "%s, %s", formatedDurationMilli(videoCachedDuration), formatedSize(videoCachedBytes)));
                    setRowValue(R.string.endo_a_cache, String.format(Locale.US, "%s, %s", formatedDurationMilli(audioCachedDuration), formatedSize(audioCachedBytes)));
                    setRowValue(R.string.endo_load_cost, String.format(Locale.US, "%d ms", mLoadCost));
                    setRowValue(R.string.endo_seek_cost, String.format(Locale.US, "%d ms", mSeekCost));
                    setRowValue(R.string.endo_seek_load_cost, String.format(Locale.US, "%d ms", seekLoadDuration));
                    setRowValue(R.string.endo_tcp_speed, String.format(Locale.US, "%s", formatedSpeed(tcpSpeed, 1000)));
                    setRowValue(R.string.endo_bit_rate, String.format(Locale.US, "%.2f kbs", bitRate/1000f));
                    setRowValue(R.string.endo_rtp_speed, String.format(Locale.US, "%s", formatedSpeed(rtpSpeed, 1000)));
                    setRowValue(R.string.endo_rtp_byte_count, String.format(Locale.US, "%s", formatedSize(rtpByteCount)));
                    setRowValue(R.string.endo_connected_time, String.format(Locale.US, "%s", formattedTime(connectedTime)));
